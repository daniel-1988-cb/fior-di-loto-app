...
export default function RedeemClient({
  ...
  async function handleRedeem(clientId: string) {
    ...
    if (!rewardId) {
      toast.info("Nessun premio disponibile o configurato.");
      return;
    }
    ...
    const ok = await confirm({
      title: `Riscuotere "${reward.nome}"?`,
      message: `-${reward.costo_punti} punti per ${client.nome} ${client.cognome}`,
      confirmLabel: "Riscuoti",
    });
    if (!ok) return;
    startTransition(async () => {
      try {
        await redeemReward(clientId, rewardId);
        toast.success(`Premio "${reward.nome}" riscosso.`);
        router.refresh();
      } catch (e) {
