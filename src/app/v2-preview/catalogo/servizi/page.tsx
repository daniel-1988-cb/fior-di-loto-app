...
export default async function V2CatalogoServiziPage({ searchParams }: PageProps) {
  ...
  return (
    <V2Shell subNav={{ title: "Catalogo", groups: catalogoSubNav }} maxWidth="full">
      ...
      <div className="grid gap-6 lg:grid-cols-[220px_1fr]">
        <aside className="rounded-xl border border-border bg-card/40 p-3">
          ...
          <Link
            href="/v2-preview/catalogo/servizi"
          ...
          {categories.map((cat) => (
            <Link
              key={cat}
              href={`/v2-preview/catalogo/servizi?cat=${encodeURIComponent(cat)}`}
              className={`flex items-center justify-between rounded-lg px-3 py-2 text-sm transition-colors ${
